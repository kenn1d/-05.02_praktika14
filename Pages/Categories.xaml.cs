...
namespace praktika14.Pages
{
    ...
    public partial class Categories : Page
    {
        ...
        {
            InitializeComponent();
            if (Classes.Basket.AllItems.Count == 0)
            {
                ...
                foreach (Classes.Item item in category_1)
                {
                    AllItems.Add(item);
                }
                foreach (Classes.Item item in category_2)
                {
                    AllItems.Add(item);
                }
                foreach (Classes.Item item in category_3)
                {
                    AllItems.Add(item);
                }

                Basket.AllItems = AllItems;

                // Добавляем категории в коллекцию
                Repo.categories.Add(new Classes.Category("Диваны и кресла", category_1));
                Repo.categories.Add(new Classes.Category("Мебель хранения", category_2));
                Repo.categories.Add(new Classes.Category("Небольшая мебель", category_3));
            }
        ...
        public void LoadItems()
        {
            ...
            foreach (Classes.Category category in Repo.categories)
            {
